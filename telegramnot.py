...
async def analyze_symbol(
    ...
    try:
        ...
        price_change_last_two_candles = ((prices[-1] - prices[-3]) / prices[-3]) * 100
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            high_price_change_coins = [
                ...
                if (
                    ((res.price_change > 2 and res.price_change_last_two_candles > 2) or res.price_change_last_two_candles > 2) and res.price_change < 9
                    and res.volume_change > 700
...
def create_table(results: List[SymbolAnalysisResult], last_updated: str, args: argparse.Namespace) -> Table:
    ...
    table.add_column("Green Candle", style="magenta", no_wrap=True)
    table.add_column("Side Taker", style="magenta", no_wrap=True)
    table.add_column("Amount Bought (USDT)", style="magenta", no_wrap=True)  # New column
    table.add_column("Amount Sold (USDT)", style="magenta", no_wrap=True)  # New column
    ...
    for res in results:
        ...
        table.add_row(
            ...
            last_candle_green_display,
            side_taker_display,
            amount_bought_display,  # New column
            amount_sold_display  # New column
        )
    return table
...
class SymbolAnalysisResult:
    ...
    median_volume_last_two_candles_usdt: float  # Median volume of the last two candles in USDT
    side_taker: str  # Indicates if the last candle is buyer or seller dominated
    amount_bought: float  # New field: Amount bought in the last candle
    amount_sold: float  # New field: Amount sold in the last candle
...
async def analyze_symbol(
    ...
    try:
        ...
        amount_bought = taker_buy_volume * last_candle_close_price  # Amount bought in USDT
        amount_sold = taker_sell_volume * last_candle_close_price  # Amount sold in USDT
        ...
        return SymbolAnalysisResult(
            ...
            median_volume_last_two_candles_usdt=median_volume_last_two_candles_usdt,
            side_taker=side_taker,
            amount_bought=amount_bought,  # New field
            amount_sold=amount_sold  # New field
        )
    except Exception as e:
        console.print(f"[red]Error analyzing symbol {symbol}: {e}[/red]")
        return None
...
def create_table(results: List[SymbolAnalysisResult], last_updated: str, args: argparse.Namespace) -> Table:
    ...
    for res in results:
        ...
        last_candle_green = res.last_candle_green
        side_taker = res.side_taker
        amount_bought = res.amount_bought  # New field
        amount_sold = res.amount_sold  # New field
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            high_price_change_coins = [
                res for res in final_results
                if (
                    ((res.price_change > 2.4 and res.price_change_last_two_candles > 2) or res.price_change_last_two_candles > 2.1)
                    and res.volume_change > 700
                    and res.last_candle_green  # Ensure the last candle is green
                    and res.last_candle_volume_usdt > 100000  # Ensure last candle volume is above 100,000 USDT
                )
...
class SymbolAnalysisResult:
    ...
    side_taker: str  # New field: Indicates if the last candle is buyer or seller dominated
...
async def analyze_symbol(
    ...
    try:
        ...
        return SymbolAnalysisResult(
            ...
            median_volume_previous_candles_usdt=median_volume_previous_candles_usdt,
            median_volume_last_two_candles_usdt=median_volume_last_two_candles_usdt,
            side_taker=side_taker  # New field
        )
    except Exception as e:
        console.print(f"[red]Error analyzing symbol {symbol}: {e}[/red]")
        return None
...
def create_table(results: List[SymbolAnalysisResult], last_updated: str, args: argparse.Namespace) -> Table:
    ...
    table.add_column("Last Candle Volume (USDT)", style="magenta", no_wrap=True)
    table.add_column("Green Candle", style="magenta", no_wrap=True)
    table.add_column("Side Taker", style="magenta", no_wrap=True)  # New column
    ...
    for res in results:
        ...
        last_candle_volume_usdt = res.last_candle_volume_usdt
        last_candle_green = res.last_candle_green
        side_taker = res.side_taker  # New field
...
async def main(args: argparse.Namespace):
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            high_price_change_coins = [
                res for res in final_results
                if ((res.price_change > 2.4 and res.price_change_last_two_candles > 2) or res.price_change_last_two_candles > 2.1) 
                and res.volume_change > 700 and res.last_candle_green and res.last_candle_volume_usdt > 100000
            ]
...
def send_telegram_notification(title: str, coins: List[SymbolAnalysisResult]):
    ...
    for coin in coins:
        message += (
            ...
            f"Price Change: {coin.price_change:.2f}%\n"
            f"Last Candle Volume (USDT): {format_volume(coin.last_candle_volume_usdt)}\n"
            f" Price Change (Last 2): {coin.price_change_last_two_candles}\n"
...
async def main(args: argparse.Namespace):
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            high_price_change_coins = [
                res for res in final_results
                if ((res.price_change > 2.4 and res.price_change_last_two_candles > 2) or res.price_change_last_two_candles > 2) and res.volume_change > 700
            ]
...
async def main(args: argparse.Namespace):
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            high_price_change_coins = [
                res for res in final_results
                if ((res.price_change > 2.4 and res.price_change_last_two_candles >2 ) or res.price_change_last_two_candles >2.1) and res.volume_change>770
            ]

            # Send push notification if any coin meets the condition
            if high_price_change_coins:
                pump_symbols = [res.symbol for res in high_price_change_coins]
                send_telegram_notification(title="🚀 Pump Detected!", message=f"3m: \n {', '.join(pump_symbols)  }")
