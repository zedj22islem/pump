...
if __name__ == '__main__':
    ...
    parser.add_argument('--wait', type=int, default=900, help='Interval for continuous monitoring mode')
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            high_price_change_coins = [
                res for res in final_results
                if (
                    ((res.price_change > 2 and res.price_change_last_two_candles > 2) or res.price_change_last_two_candles > 2) 
                    and res.volume_change > 450
             #       and res.last_candle_green  # Ensure the last candle is green
                    and res.last_candle_volume_usdt > 100000  # Ensure last candle volume is above 100,000 USDT
                )
            ]
...
async def main(args: argparse.Namespace):
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            high_price_change_coins = [
                res for res in final_results
                if (res.price_change > 2.4 and res.price_change_last_two_candles >1.2 )or res.price_change_last_two_candles >2.1
            ]

            # Send push notification if any coin meets the condition
            if high_price_change_coins:
                pump_symbols = [res.symbol for res in high_price_change_coins]
                send_telegram_notification(title="🚀 Pump Detected!", message=f"15 min: \n {', '.join(pump_symbols)}")
