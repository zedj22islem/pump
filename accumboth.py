...
lookback_days = 4  # Lookback period for price range and volume analysis
...
for coin in coins:
    try:
        ...
        if range_condition:
            ...
            last_candle_volume = df.iloc[-1]["volume"]

            # Get the highest and lowest prices in the lookback period
            highest_price = df["high"].max()
            lowest_price = df["low"].min()

            # Add only coins with Buy Side dominance
...
if range_bound_coins:
    print("Coins likely bouncing in a range with Buy Side dominance:")
...
def analyze_volume(df):
    """Calculate dominance based on taker buy and sell volumes."""
    total_buy_volume = df["taker_buy_quote"].sum()  # Total taker buy volume in USDT
    total_sell_volume = df["volume_usdt"].sum() - total_buy_volume  # Total taker sell volume in USDT
...
coins = [   'EOSUSDT', 'IOTAUSDT', 'XLMUSDT', 'ONTUSDT', 'ETCUSDT', 'ICXUSDT', 'NULSUSDT', 'VETUSDT', 'LINKUSDT', 'ONGUSDT',
    ...
    'NOTUSDT', 'IOUSDT', 'ZKUSDT', 'ZROUSDT', 'GUSDT', 'BANANAUSDT', 'RENDERUSDT', 'TONUSDT', 'SLFUSDT', 'POLUSDT',
    'CATIUSDT', 'SCRUSDT', 'KAIAUSDT', 'ACXUSDT', 'MOVEUSDT', 'MEUSDT', 'VANAUSDT', 'BIOUSDT']  # Replace with your coin list (truncated for brevity)
lookback_days = 3  # Lookback period for price range and volume analysis
range_threshold = 0.4  # 40% range threshold
...
def analyze_volume(df):
    """Calculate average upward and downward volumes in USDT."""
    df["change"] = df["close"] - df["open"]
    df["volume_usdt"] = df["volume"] * df["close"]  # Volume in USDT
    upward_volume = df[df["change"] > 0]["volume_usdt"].mean()
    downward_volume = df[df["change"] <= 0]["volume_usdt"].mean()
    dominance = "Buy Side" if upward_volume > downward_volume else "Sell Side"
    return dominance
...
def analyze_volume(df):
    ...
    downward_volume = df[df["change"] <= 0]["volume_usdt"].mean()
    dominance = "Buy Side" if upward_volume > downward_volume else "Sell Side"
    return upward_volume, downward_volume, dominance
...
for coin in coins:
    try:
        ...
        if range_condition:
            price_range_percentage = calculate_price_range_percentage(df)
            upward_vol, downward_vol, dominance = analyze_volume(df)
            ...
            range_bound_coins.append({
                    ...
                    "Support": format_number(support),
                    "Resistance": format_number(resistance),
                    "Avg Up Volume (USDT)": format_number(upward_vol),
                    "Avg Down Volume (USDT)": format_number(downward_vol),
                })
        time.sleep(0.1)  # Avoid hitting API rate limits
    except Exception as e:
        print(f"Error analyzing {coin}: {e}")
