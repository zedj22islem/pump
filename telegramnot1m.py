        ...
        while True:
            ...
            high_price_change_coins = [
                ...
                if (
                    ...
                    and res.last_candle_volume_usdt > 70000  # Ensure last candle volume is above 100,000 USDT
...
if __name__ == '__main__':
    ...
    parser.add_argument('--interval', type=str, default="1m", help='Timeframe for volume analysis (e.g. 15m, 1h, 4h, 1d)')
    parser.add_argument('--range', type=str, default="3h", help='Time range for volume analysis (e.g. 4h, 1d, 3d)')
...
async def main(args: argparse.Namespace):
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            high_price_change_coins = [
                res for res in final_results
                if ((res.price_change > 2 and res.price_change_last_two_candles >1 ) or res.price_change_last_two_candles >2) and res.volume_change>1000
            ]

            # Send push notification if any coin meets the condition
            if high_price_change_coins:
                pump_symbols = [res.symbol for res in high_price_change_coins]
                send_telegram_notification(title="🚀 Pump Detected!", message=f"1m: \n {', '.join(pump_symbols)}")
